...
/**
 * This is an long hashmap that has the exact same features and interface as a normal Map except
 * that the key is directly a long. So no hash is calculated or key object is stored.
 *
...
public class LongHashMap<V> implements Cloneable, Serializable {
    ...
    void putAllForCreate(LongHashMap<V> m) {
     ...
     */
    public void putAll(LongHashMap<V> m) {
     ...
     */
    Entry<V> removeEntryForKey(long key) {
        int i = indexFor(key, table.length);
    ...
    public boolean containsValue(Object value) {
        ...
        Entry<V> tab[] = table;
        for (Entry<V> aTab : tab) {
            for (Entry<V> e = aTab; e != null; e = e.next) {
    ...
    public Object clone() throws CloneNotSupportedException {
        LongHashMap<V> result = null;
        try {
            result = (LongHashMap<V>) super.clone();
...
//        }
    ...
    private class KeySet extends AbstractSet<Long> {
        ...
        public boolean remove(Object o) {
            if (o instanceof Number) {
                return removeEntryForKey(((Number) o).longValue()) != null;
            }
            return false;
        }
